...
class LOOTLOCKERADMINSDKEDITOR_API ULootLockerAdminHttpClient : public UObject
{
	GENERATED_BODY()
public:
    static ULootLockerAdminHttpClient& GetInstance();
    template<typename ResponseType>
    struct ResponseInspector
    {
        DECLARE_DELEGATE_OneParam(FLootLockerAdminResponseInspectorCallback, ResponseType&);
    };
    template<typename ResponseType, typename RequestType, typename BlueprintDelegate, typename CppDelegate>
    static void SendRequest(RequestType Request, FLootLockerAdminEndPoint Endpoint, const TArray<FStringFormatArg>& InOrderedArguments, const TMultiMap<FString, FString> QueryParams, const BlueprintDelegate& OnCompletedRequestBP, const CppDelegate& OnCompletedRequest, const typename ResponseInspector<ResponseType>::FLootLockerAdminResponseInspectorCallback& ResponseInspectorCallback = typename ResponseInspector<ResponseType>::FLootLockerAdminResponseInspectorCallback(), TMap<FString, FString> CustomHeaders = TMap<FString, FString>())
	{
    ...
    static void SendRawRequest(FString RequestBody, FLootLockerAdminEndPoint Endpoint, const TArray<FStringFormatArg>& InOrderedArguments, const TMultiMap<FString, FString> QueryParams, const BlueprintDelegate& OnCompletedRequestBP, const CppDelegate& OnCompletedRequest, const typename ResponseInspector<ResponseType>::FLootLockerAdminResponseInspectorCallback& ResponseInspectorCallback = typename ResponseInspector<ResponseType>::FLootLockerAdminResponseInspectorCallback(), TMap<FString, FString> CustomHeaders = TMap<FString, FString>())
    {
    ...
    template<typename ResponseType, typename BlueprintDelegate, typename CppDelegate>
    static void SendRawWWWFormUrlEncodedRequest(FString RequestBody, FLootLockerAdminEndPoint Endpoint, const TArray<FStringFormatArg>& InOrderedArguments, const TMultiMap<FString, FString> QueryParams, const BlueprintDelegate& OnCompletedRequestBP, const CppDelegate& OnCompletedRequest, const typename ResponseInspector<ResponseType>::FLootLockerAdminResponseInspectorCallback& ResponseInspectorCallback = typename ResponseInspector<ResponseType>::FLootLockerAdminResponseInspectorCallback(), TMap<FString, FString> CustomHeaders = TMap<FString, FString>())
    {
    ...
    template<typename ResponseType, typename BlueprintDelegate, typename CppDelegate>
    static void UploadFile(const FString& FilePath, const TMap<FString, FString> AdditionalFields, FLootLockerAdminEndPoint Endpoint, const TArray<FStringFormatArg>& InOrderedArguments, const TMultiMap<FString, FString> QueryParams, const BlueprintDelegate& OnCompletedRequestBP, const CppDelegate& OnCompletedRequest, const typename ResponseInspector<ResponseType>::FLootLockerAdminResponseInspectorCallback& ResponseInspectorCallback = typename ResponseInspector<ResponseType>::FLootLockerAdminResponseInspectorCallback(), TMap<FString, FString> CustomHeaders = TMap<FString, FString>()) 
    {
        GetInstance().UploadFile_Internal(FilePath, AdditionalFields, TemplatedHTTPRequest<ResponseType>::Make(FLootLockerAdminEmptyRequest{}, Endpoint, InOrderedArguments, QueryParams, OnCompletedRequestBP, OnCompletedRequest, ResponseInspectorCallback, CustomHeaders));
    }

    template<typename ResponseType, typename BlueprintDelegate, typename CppDelegate>
    static void UploadRawFile(const TArray<uint8>& RawData, const FString& FileName, const TMap<FString, FString> AdditionalFields, FLootLockerAdminEndPoint Endpoint, const TArray<FStringFormatArg>& InOrderedArguments, const TMultiMap<FString, FString> QueryParams, const BlueprintDelegate& OnCompletedRequestBP, const CppDelegate& OnCompletedRequest, const typename ResponseInspector<ResponseType>::FLootLockerAdminResponseInspectorCallback& ResponseInspectorCallback = typename ResponseInspector<ResponseType>::FLootLockerAdminResponseInspectorCallback(), TMap<FString, FString> CustomHeaders = TMap<FString, FString>())
    {
        GetInstance().UploadRawFile_Internal(RawData, FileName, AdditionalFields, TemplatedHTTPRequest<ResponseType>::Make(FLootLockerAdminEmptyRequest{}, Endpoint, InOrderedArguments, QueryParams, OnCompletedRequestBP, OnCompletedRequest, ResponseInspectorCallback, CustomHeaders));
    }

private:
    ULootLockerAdminHttpClient();
    ...
    struct HTTPRequest
    {
        ...
        FString ContentType = "";
        FString Data = "";
        FLootLockerAdminResponseCallback OnCompleteRequest;
    ...
    static ULootLockerAdminHttpClient* Instance;
    static bool ResponseIsSuccess(const FHttpResponsePtr& InResponse, bool bWasSuccessful);
    static void LogFailedRequestInformation(const FLootLockerAdminResponse& Response, const FString& RequestMethod, const FString& Endpoint, const FString& Data, const
                                            TArray<FString>& ResponseHeaders);
	static void LogSuccessfulRequestInformation(const FLootLockerAdminResponse& Response, const FString& RequestMethod, const FString& Endpoint, const FString& Data, const
	                                            TArray<FString>& ResponseHeaders);
    ...
    struct TemplatedHTTPRequest
    {
        ...
        static FLootLockerAdminResponseCallback CreateLambda(const BluePrintDelegate& OnCompletedRequestBP, const CppDelegate& OnCompletedRequest, const typename ResponseInspector<ResponseType>::FLootLockerAdminResponseInspectorCallback& ResponseInspectorCallback)
        {
            return FLootLockerAdminResponseCallback::CreateLambda([OnCompletedRequestBP, OnCompletedRequest, ResponseInspectorCallback](FLootLockerAdminResponse response)
            {
        ...
        static HTTPRequest MakeRaw(FString ContentString, FLootLockerAdminEndPoint Endpoint, const TArray<FStringFormatArg>& InOrderedArguments, const TMultiMap<FString, FString> QueryParams, const BluePrintDelegate& OnCompletedRequestBP, const CppDelegate& OnCompletedRequest, const typename ResponseInspector<ResponseType>::FLootLockerAdminResponseInspectorCallback& ResponseInspectorCallback = ResponseInspector<ResponseType>::FLootLockerAdminResponseInspectorCallback::CreateLambda([](const ResponseType& Ignored) {}), TMap<FString, FString> CustomHeaders = TMap<FString, FString>(), const FString& ContentType = "")
        {
            ...
            EndpointWithArguments = LootLockerAdminUtilities::AppendParametersToUrl(EndpointWithArguments, QueryParams);

            // create callback lambda
            const FLootLockerAdminResponseCallback SessionResponse = CreateLambda<BluePrintDelegate, CppDelegate>(OnCompletedRequestBP, OnCompletedRequest, ResponseInspectorCallback);
        ...
        static HTTPRequest Make(RequestType RequestStruct, FLootLockerAdminEndPoint Endpoint, const TArray<FStringFormatArg>& InOrderedArguments, const TMultiMap<FString, FString> QueryParams, const BluePrintDelegate& OnCompletedRequestBP, const CppDelegate& OnCompletedRequest, const typename ResponseInspector<ResponseType>::FLootLockerAdminResponseInspectorCallback& ResponseInspectorCallback = ResponseInspector<ResponseType>::FLootLockerAdminResponseInspectorCallback::CreateLambda([](const ResponseType& Ignored) {}), TMap<FString, FString> CustomHeaders = TMap<FString, FString>(), const FString& ContentType = "")
        {
...
#else
            if (!std::is_same_v<RequestType, FLootLockerAdminEmptyRequest>)
            {
                FJsonObjectConverter::UStructToJsonObjectString(RequestType::StaticStruct(), &RequestStruct, ContentString, 0, 0);
            }
#endif
